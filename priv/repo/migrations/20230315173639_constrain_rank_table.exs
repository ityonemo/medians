defmodule Db.Repo.Migrations.ConstrainRankTable do
  ...
  def up do
    ...
    create constraint(:ranks, :rank_must_be_positive, check: "upper(rank) > 0")

    create constraint(:ranks, :no_overlap, exclude: ~s|gist(year WITH =, rank WITH &&)|)
  end

  def down do
    drop index(:ranks, [:one_rank_per_year])
    drop constraint(:ranks, [:rank_must_be_positive, :no_overlap])
